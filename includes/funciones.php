...
function correo($mail, $nombre, $movil, $email,$fecha, $hora, $horafinal, $servicio, $crear,$rol){
      //configurar SMTP
      $mail->isSMTP();
      $mail->Host='smtp.titan.email';  //especifico de gmail
      $mail->SMTPAuth=true;
      $mail->Username='proyecto@citasopticajose.online';
      $mail->Password='';
      $mail->SMTPSecure='tls';   //canal seguro
      $mail->Port=465;

      //configurar el contenido de email
      $mail->setFrom('proyecto@citasopticajose.online', 'OpticaCitas'); //quien envia el email
      if($rol){
        $mail->addAddress('proyecto@citasopticajose.online', 'OpticaCitas'); //quien le llega o recibe
      }else{
...
function correoAlta($mail, $correo, $password, $alta){
   ...
   $mail->Port=465;
    ...
    if($alta){
          ...
          $mail->addAddress('proyecto@citasopticajose.online', 'OpticaCitas'); //quien recibe
    }
